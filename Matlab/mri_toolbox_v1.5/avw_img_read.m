...
if fid < 0,
    ...
    avw = read_image(fid,fileprefix,IMGorient,machine);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ avw ] = read_image(fid,fileprefix,IMGorient,machine)
	
	% Read the file header
	[ avw, machine ] = avw_hdr_read(fileprefix,machine);
	
	fprintf('AVW_IMG_READ...\n');  tic;
    ...
    PixelSz  = avw.hdr.dime.pixdim(2);
    RowSz    = avw.hdr.dime.pixdim(3);
    SliceSz  = avw.hdr.dime.pixdim(4);
    
    
    % --- check the orientation specification and arrange img accordingly
    if ~isempty(IMGorient), avw.hdr.hist.orient = IMGorient; end,
    ...
    case 1, % coronal unflipped
        ...
        avw.hdr.dime.dim(2:4) = [int16(PixelDim),int16(SliceDim),int16(RowDim)];
        avw.hdr.dime.pixdim(2:4) = [PixelSz,SliceSz,RowSz];
    ...
    case 2, % sagittal unflipped
        ...
        % rearrange avw.hdr.dime.dim or avw.hdr.dime.pixdim
        avw.hdr.dime.dim(2:4) = [int16(SliceDim),int16(PixelDim),int16(RowDim)];
        avw.hdr.dime.pixdim(2:4) = [SliceSz,PixelSz,RowSz];
