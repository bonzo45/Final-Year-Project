...
%
% [ avw_hdr, machine ] = avw_hdr_read(fileprefix, machine)
%
...
if fid < 0,
    msg = sprintf('...cannot open file %s.hdr\n\n',fileprefix);
    error(msg);
else
    fclose(fid);
    fprintf('...reading %s Analyze format.\n',machine);
    ...
    if ~isequal(avw.hdr.hk.sizeof_hdr,348),
        fprintf('...failed  %s Analyze format.\n',machine);
    ...
    if ~isequal(avw.hdr.hk.sizeof_hdr,348),
        % Now echo warning
        fprintf('...failed reading %s Analyze format.\n',machine);
