...
The idea behind the uncertainty surface is to project the uncertainty (a 3D volume) onto a surface (effectively 2D) to give an overview of the uncertainty hotspots. Two versions have been implemented; the first maps the uncertainty to a sphere and the second maps the uncertainty to a surface representation of the organ being scanned.

The uncertainty sphere is a generic visualization that can be applied to any scan, whereas the second requires a surface representation of the organ in question. Both use the same technique; for each point (on the sphere/surface) a ray is fired into the uncertainty volume to accumulate the uncertainty (see volume rendering section \ref{background:volumerendering}) and the result is mapped to a colour.
...
\newpage
\chapter{Tool}
Tool. Research. Prototype. Pipeline reconstruction. Easy. Integrated. Visualization + Two features. Scan simulation. Reconstruction. Visualization. Mainly to evaluate thoughts.

Nice screenshot of MITK. Annotated.

\newpage
\section{Scan Simulation}\label{section:simulatescan}
Treat reconstruction as ground truth. Rescan from different angles, different resolutions. Simulate motion. Naive. Use to test reconstruction algorithms.

Simulation screenshot. Explain each control. Show example scan. Explain that this is yet to be tested.

\newpage
\section{Reconstruction}\label{section:reconstruction}
Call GPU code. In tool. Add landmarks. Help reconstruction. Mainly evaluate thoughts.

Show screenshot (of landmarks). Explain that this was just to prompt opinions and gauge how long it takes.

\section{Visualization}\label{sectiontoolvisualization}
