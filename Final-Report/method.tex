...
As well as allowing the user to explicitly specify a range [min, max], it is possible to find the worst x$\%$ of uncertainty. To do this a histogram is built which splits the uncertainty into buckets. The histogram is then traversed, starting from the first (least certain) bucket, and the occurences are accumulated until the total is less than $total_pixels * \frac{x}{100}$.
